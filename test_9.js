        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Verificar si hay un ID de vacante en la URL
            const urlParams = new URLSearchParams(window.location.search);
            const vacanteId = urlParams.get('id');
            
            if (vacanteId) {
                // Cargar vacante existente
                console.log('📋 [DOMContentLoaded] Cargando vacante con ID:', vacanteId);
                loadVacanteFromStorage(vacanteId);
            } else {
                // Nueva vacante - inicializar directamente en paso 2 (flujo del candidato)
                console.log('📋 [DOMContentLoaded] Nueva vacante - inicializando flujo del candidato');
                initializeNewVacante();
            }
        ...
        let isEditingVacanteWithIATemplate = false; // Indica si estamos editando una vacante con plantilla IA (solo desde index.html)
        let isEditingVacanteWithDefaultTemplate = false; // Indica si estamos editando una vacante con plantilla por defecto (IA o estándar) desde index.html
        let isEditingSpecificVacante = false; // Indica si estamos editando la vacante específica "Desarrollador Frontend React" (vacante-1)
        ...
        function createTemplateSelect(initialValue = 'custom') {
            // Cargar plantillas y actualizar la variable global
        ...
        function selectTemplate(templateId) {
            ...
            if (templateId === 'custom') {
                ...
                // Asegurar que NO estamos editando una vacante con plantilla por defecto
                isEditingVacanteWithIATemplate = false;
                isEditingVacanteWithDefaultTemplate = false;
                ...
                // Asegurar que las etapas estén cargadas antes de renderizar
                if (availableStages.length === 0 && typeof loadAvailableStages === 'function') {
                    loadAvailableStages();
                }
                ...
                // Configurar drag and drop y renderizar biblioteca después de renderizar
                setTimeout(() => {
                    // Configurar drag and drop
                    if (typeof makeBoardDroppable === 'function') {
                        makeBoardDroppable();
                    }
                    ...
                    if (activeTab) {
                        ...
                        if (tabType === 'stages') {
                            if (typeof renderStages === 'function') {
                                renderStages();
                            } else if (typeof renderAvailableStages === 'function') {
                                renderAvailableStages();
                            }
                        } else {
                            if (typeof renderAgents === 'function') {
                                renderAgents();
                            } else if (typeof window.renderAgents === 'function') {
                                window.renderAgents();
                            }
                        }
                    } else {
                        // Si no hay tab activo, renderizar etapas por defecto
                        if (typeof renderStages === 'function') {
                            renderStages();
                        } else if (typeof renderAvailableStages === 'function') {
                            renderAvailableStages();
            ...
            // Si estamos editando una vacante con plantilla por defecto, forzar modo edición
            if (isEditingVacanteWithDefaultTemplate) {
                isEditMode = true;
            }
            ...
            // Si es la vacante específica, forzar modo vista (no edición)
            if (isEditingSpecificVacante) {
                isEditMode = false;
            } else if (isEditingVacanteWithDefaultTemplate) {
                // Si estamos editando una vacante con plantilla por defecto (pero no la específica), mantener modo edición
                isEditMode = true;
            } else {
                // De lo contrario, cargar en modo vista (no edición)
                isEditMode = false;
            }
            ...
            if (libraryWidget) {
                if (isEditingVacanteWithDefaultTemplate && !isEditingSpecificVacante) {
                    libraryWidget.style.display = 'flex'; // Mostrar biblioteca en modo edición
                } else {
            ...
            if (flowSection) {
                if (isEditingVacanteWithDefaultTemplate && !isEditingSpecificVacante) {
                    flowSection.className = 'section-dual'; // Mostrar biblioteca y flujo en modo edición
                } else {
            ...
            renderTemplateFlow(localTemplateCopy);
            // Aplicar modo edición/vista después de renderizar
            applyEditMode();
            
            // Si estamos en modo edición (pero no es la vacante específica), renderizar contenido de la biblioteca
            if (isEditingVacanteWithDefaultTemplate && !isEditingSpecificVacante) {
                setTimeout(() => {
                    const activeTab = document.querySelector('#agents-sidebar .tab-button.active');
                    if (activeTab) {
        ...
        function renderTemplateFlow(template) {
            ...
            // Alerta de información cuando se edita vacante con plantilla IA (solo si hay etapas con IA)
            let defaultTemplateAlertHTML = '';
            if (isEditingVacanteWithIATemplate) {
                // Verificar si hay etapas con IA en el template
                const hasAgentStages = template.realContent && template.realContent.stages && 
                    template.realContent.stages.some(stage => stage.type === 'agent' && stage.agentId);
                
                // Solo mostrar alerta si hay etapas con IA
                if (hasAgentStages) {
                    // Mensaje diferente para la vacante específica
                    const alertMessage = isEditingSpecificVacante
                        ? 'Este flujo contiene etapas con IA. Por el momento no es posible editarlo porque afectaría todos los datos ya recolectados en el proceso de selección.'
                        : 'Las etapas con IA no se pueden editar, configurar ni eliminar. Solo puedes cambiar su orden arrastrándolas.';
                    
                    defaultTemplateAlertHTML = `
                        <div class="ubits-alert ubits-alert--info ubits-alert--no-close" style="margin-top: 16px;">
                            <div class="ubits-alert__icon">
                                <i class="far fa-circle-info"></i>
                            </div>
                            <div class="ubits-alert__content">
                                <div class="ubits-alert__text">${alertMessage}</div>
                            </div>
                        </div>
                    `;
                }
            ...
            if (selectedTemplate) {
                ...
                // NO mostrar botón "Editar flujo" si estamos editando una vacante con plantilla IA (ya está en modo edición)
                headerActionsHTML = `
                    <div class="flow-header-actions">
                        ...
                        ` : ''}
                        ${!isEditingVacanteWithDefaultTemplate && !isEditingSpecificVacante ? `
        ...
        function applyEditMode() {
            ...
            if (isEditingSpecificVacante) {
                ...
                // Si estamos editando una vacante con plantilla por defecto (pero no la específica), forzar modo edición
                isEditMode = true;
            }
            ...
            if (isEditMode) {
                ...
                // Ocultar botón "Editar flujo" si estamos editando una vacante con plantilla IA
                if (editFlowBtn) {
                    if (isEditingVacanteWithIATemplate) {
                        editFlowBtn.style.display = 'none';
                    } else {
                        editFlowBtn.style.display = '';
        ...
        window.toggleEditMode = function() {
            // Prevenir cambiar modo si estamos editando una vacante con plantilla por defecto o la vacante específica
            if (isEditingVacanteWithDefaultTemplate || isEditingSpecificVacante) {
                return;
            }
        ...
        window.goToTemplate = function() {
            if (selectedTemplate) {
                // Abrir en una nueva pestaña para no salir de la página de vacantes
                window.open(`editor-plantillas.html?id=${selectedTemplate.id}`, '_blank');
            }
        };
        ...
        function renderAgentStageCard(stage, index, template) {
            ...
            const agentDescriptions = {
                ...
                'onboarding': 'Acompaña la incorporación del nuevo colaborador'
            };
            ...
            // NO mostrar configuración si estamos editando una vacante con plantilla IA
            let configHTML = '';
            if (isEditMode && !isEditingVacanteWithIATemplate && agentData.hasConfig && agentData.config && stage.agentId !== 'psychometric-analyst' && stage.agentId !== 'interview-ia') {
                configHTML = `
                    ...
                    <div class="stage-content">
                        <div class="stage-header agent-stage-header">
                            ...
                            <div class="stage-actions">
                                ${!isEditingVacanteWithIATemplate ? `
                                    ...
                                    ` : agentData.hasConfig && stage.agentId === 'onboarding' ? `
                        ...
                        ${isEditMode && stage.agentId === 'onboarding' ? `
                            <div class="stage-description" id="agent-description-${stage.id}">
                                ${agentDescription}
                            </div>
                        ` : ''}
                    </div>
        ...
        window.toggleAgentStageConfig = function(stageId) {
            ...
            // Prevenir configuración solo si estamos editando una vacante con plantilla IA
            if (isEditingVacanteWithIATemplate) {
                if (typeof showToast === 'function') {
                    showToast('info', 'No se puede configurar etapas con IA en vacantes con plantilla IA.');
                }
                return;
            }
            ...
            // Etapas sin configuración desplegable (ej. Onboarding) muestran su
            // descripción inline en lugar del panel de configuración
            const descDiv = document.getElementById(`agent-description-${stageId}`);
            if (descDiv) {
                descDiv.style.display = stage.expanded ? 'block' : 'none';
            }
            ...
            if (chevron) {
                if (configDiv) {
                    ...
                    // Chevron de expandir/contraer descripción (ej. Onboarding)
                    chevron.className = stage.expanded ? 'far fa-chevron-up' : 'far fa-chevron-down';
                }
            }
        };
        ...
        window.deleteAgentStage = function(stageId) {
            ...
            // Prevenir eliminación solo si estamos editando una vacante con plantilla IA y la etapa es con IA
            if (isEditingVacanteWithIATemplate) {
                const stage = localTemplateCopy.realContent.stages.find(s => s.id === stageId);
                const isAgent = stage && stage.type === 'agent' && stage.agentId;
                
                // Solo prevenir si es una etapa con IA
                if (isAgent) {
                    if (typeof showToast === 'function') {
                        showToast('info', 'No se pueden eliminar etapas con IA en vacantes con plantilla IA. Solo puedes cambiar su orden.');
                    }
                    return;
                }
            }
        ...
        // Inicializar nueva vacante
        function initializeNewVacante() {
            console.log('📋 [initializeNewVacante] Inicializando nueva vacante');
            
            // Inicializar estado de la vacante primero
            selectedTemplate = null;
            localTemplateCopy = null;
            isEditMode = true;
            isEditingVacanteWithIATemplate = false;
            ...
            // Asegurar que las etapas estén cargadas
            if (availableStages.length === 0 && typeof loadAvailableStages === 'function') {
                loadAvailableStages();
            }
            
            // Crear input de plantillas
            setTimeout(() => {
                createTemplateSelect('custom');
            }, 100);
        ...
        // Cargar vacante existente desde localStorage
        function loadVacanteFromStorage(vacanteId) {
            console.log('📋 [loadVacanteFromStorage] Cargando vacante con ID:', vacanteId);
            
            try {
                // Cargar todas las vacantes
                const stored = localStorage.getItem('vacantes');
                if (!stored) {
                    console.error('❌ [loadVacanteFromStorage] No hay vacantes guardadas');
                    initializeNewVacante();
                    return;
                }
                
                const vacantes = JSON.parse(stored);
                const vacante = vacantes.find(v => v.id === vacanteId);
                
                if (!vacante) {
                    console.error('❌ [loadVacanteFromStorage] Vacante no encontrada:', vacanteId);
                    initializeNewVacante();
                    return;
                }
                
                console.log('✅ [loadVacanteFromStorage] Vacante encontrada:', vacante);
                
                // Detectar si es la vacante específica "Desarrollador Frontend React" (vacante-1)
                isEditingSpecificVacante = (vacanteId === 'vacante-1');
                
                // Obtener el templateId de la vacante (puede ser null o un ID de plantilla)
                const templateId = vacante.templateId || 'custom';
                const daysValue = vacante.days || '3';
                
                // Detectar si estamos editando una vacante con plantilla IA
                isEditingVacanteWithIATemplate = (templateId === 'default-template-ia');
                
                // Detectar si estamos editando una vacante con plantilla por defecto (IA o estándar)
                isEditingVacanteWithDefaultTemplate = (templateId === 'default-template-ia' || templateId === 'default-template-standard');
                ...
                console.log('📋 [loadVacanteFromStorage] Template ID:', templateId);
                console.log('📋 [loadVacanteFromStorage] Days:', daysValue);
                console.log('📋 [loadVacanteFromStorage] Is editing vacante with IA template:', isEditingVacanteWithIATemplate);
                console.log('📋 [loadVacanteFromStorage] Is editing vacante with default template:', isEditingVacanteWithDefaultTemplate);
                console.log('📋 [loadVacanteFromStorage] Is editing specific vacante (vacante-1):', isEditingSpecificVacante);
                console.log('📋 [loadVacanteFromStorage] Edit mode:', isEditMode);
                
                // Crear input de plantillas con el templateId de la vacante
                setTimeout(() => {
                    createTemplateSelect(templateId);
                }, 100);
                
                // Crear input de días con el valor guardado
                setTimeout(() => {
                    createInput({
                        containerId: 'days-input-container',
                        label: 'Enviar correo de rechazo a los: (días)',
                ...
                // Si tiene una plantilla, selectTemplate ya se llamará automáticamente desde createTemplateSelect
                // No necesitamos llamarlo manualmente aquí porque createTemplateSelect lo hace
                
            } catch (error) {
                console.error('❌ [loadVacanteFromStorage] Error:', error);
                // Si hay error, inicializar como nueva vacante
                initializeNewVacante();
            }
        }
        ...
        window.openPsychometricTestsDrawer = function(stageId) {
            ...
            // Prevenir abrir pruebas solo si estamos editando una vacante con plantilla IA
            if (isEditingVacanteWithIATemplate) {
                if (typeof showToast === 'function') {
                    showToast('info', 'No se pueden configurar pruebas en etapas con IA cuando se edita una vacante con plantilla IA.');
                }
                return;
            }
...
;
        ...
        window.openSerenaConfigDrawer = function(stageId) {
            ...
            // Prevenir abrir configuración solo si estamos editando una vacante con plantilla IA
            if (isEditingVacanteWithIATemplate) {
                if (typeof showToast === 'function') {
                    showToast('info', 'No se puede configurar etapas con IA cuando se edita una vacante con plantilla IA.');
                }
                return;
            }
