...
# I will find the broken setInterval string and replace it with a fixed one
# The current one starts with `let count=0; let intv = setInterval(()=>{count++; if(count>20) clearInterval(intv);`

# We need to construct the regex to match everything up to the `}, 250);`
pattern = r'let count=0; let intv = setInterval\(\(\)=>\{count\+\+; if\(count>20\) clearInterval\(intv\);.*?\}, 250\);'
...
new_timeout_hack = r'''let count=0; let intv = setInterval(()=>{
  ...
  document.querySelectorAll("div.rounded-xl.border.overflow-hidden").forEach(card => {
    if(card.textContent.includes("Esperando acción manual") || card.dataset.hacked === "true") {
      // Update badge ONLY if not already hacked
      if (card.dataset.hacked !== "true") {
